...
async def verify_discord_key(
    ...
    try:
        ...
        message = x_signature_timestamp.encode() + await req.body()
...
async def webhook_discord(interaction: DiscordInteraction):
    """
    Receive Discord webhook
    """
    logger.info("Received Discord webhook request for type: %s", interaction.type)
